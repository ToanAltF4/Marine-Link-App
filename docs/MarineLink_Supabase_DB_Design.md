...
| Authorization model | Role-based với `roles` và `user_roles` |
...
- Các bảng join thuần như `user_roles` không cần `public_id` nếu không trả trực tiếp như resource API.
...
Lưu thông tin tài khoản. Role không đặt trực tiếp trong bảng này để tránh khó mở rộng; role được gán qua `user_roles`.
...
Gán nhiều role cho một user. MVP thường mỗi user có một role chính, nhưng bảng này vẫn cho phép một tài khoản có nhiều role nếu cần.

| Column | Type | Constraint | Note |
|---|---|---|---|
| user_id | bigint | FK -> users.id, not null | User được gán role |
| role_id | bigint | FK -> roles.id, not null | Role được gán |
| assigned_by | bigint | FK -> users.id, nullable | Admin gán role |
| assigned_at | timestamptz | not null, default now() | Lúc gán |
...
| user_roles | Không truy cập trực tiếp | Read role user phục vụ support nếu backend cho phép | Full gán role |
...
Nếu không dùng Supabase Auth, RLS trực tiếp theo `auth.uid()` chưa map được với `users.id`. Khi đó Spring Boot nên truy cập DB bằng backend DB role và tự enforce quyền ở service layer. Role lấy từ `user_roles` -> `roles`. Nếu sau này dùng Supabase Auth, cần thêm `auth_user_id uuid` trong `users` để map với `auth.users.id`.
...
| Admin lọc user role/status | `user_roles(role_id)` + `roles(code)` + `users(status)` |
| Product list theo category/status | `products(category_id, status)` |
...
| User có role hợp lệ | FK `user_roles.role_id` | Auth/Admin user service |
...
|---|---|---|
| `001_extensions_and_enums` | `pgcrypto` extension for `gen_random_uuid()`, enum types | Chưa làm |
| `002_identity_roles` | `users`, `roles`, `user_roles`, indexes, updated_at trigger | Chưa làm |
| `003_catalog` | `categories`, `products`, `price_tiers`, `product_images` | Chưa làm |
| `004_cart_orders` | `carts`, `cart_items`, `orders`, `order_items`, `order_status_history` | Chưa làm |
| `005_notifications` | `notifications`, related indexes | Chưa làm |
| `006_messaging_complaints` | `chat_rooms`, `chat_messages`, `chat_attachments`, `complaints` | Chưa làm |
| `007_warehouses` | `warehouses` | Chưa làm |
| `008_storage_policies` | Storage buckets/policies nếu dùng Supabase Storage | Chưa làm |
| `009_seed_demo_data` | Categories, products, price tiers, demo users | Chưa làm |
| `010_rls_optional` | RLS policies nếu mở direct Supabase access | Chưa làm |
...
- 1 Admin: `admin@marinelink.demo`, gán role `ADMIN`
- 1 Staff: `staff@marinelink.demo`, gán role `STAFF`
- 2 Đại lý: một `ACTIVE`, một `PENDING_APPROVAL`, gán role `USER`
- Categories: Mực khô, Tôm khô, Cá khô, Nước mắm
- Ít nhất 8 products, mỗi category 2 sản phẩm
- Mỗi product có 2-3 price tiers
- 2 warehouses
...
| Role model | Use `roles` and `user_roles`; không đặt role trực tiếp trong `users` để dễ mở rộng |
