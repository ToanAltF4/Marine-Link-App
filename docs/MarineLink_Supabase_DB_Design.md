...
Danh mục sản phẩm dạng cây: nhóm cha như `Cá`, `Tôm`, `Mực`; nhóm con như `Cá khô`, `Cá đông lạnh`, `Tôm khô`, `Mực khô`.

| Column | Type | Constraint | Note |
|---|---|---|---|
| id | bigint | PK, generated identity, internal only | Category ID nội bộ |
| public_id | uuid | unique, default `gen_random_uuid()` | Public ID trả qua API |
| parent_id | bigint | nullable FK -> categories.id, check khác chính nó | Danh mục cha |
...
create index categories_parent_order_idx on categories (parent_id, display_order, name);
