...
| S2-06 | P1 | Notification list: đã đọc/chưa đọc, mở order/product/chat liên quan | `GET /api/notifications`, `PUT /api/notifications/{id}/read`, tạo notification từ order event | `notifications` với owner/link target, index unread | 3 pts | Order events | Xong FE + API thật                                               |
