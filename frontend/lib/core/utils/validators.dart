...
abstract class Validators {
  ...
  static String? confirmPassword(String? value, String? original) {
    ...
    if (value != original) return 'Mật khẩu không khớp';
    return null;
  }
  ...
  // ── Tax code (Mã số thuế VN) ──────────────────────────────────────────────
  static String? taxCode(String? value) {
    if (value == null || value.trim().isEmpty) return null; // optional field
    final taxRegex = RegExp(r'^\d{10}(-\d{3})?$');
    if (!taxRegex.hasMatch(value.trim())) return 'Mã số thuế không hợp lệ';
    return null;
  }
  ...
  static String? quantity(String? value, {int minQuantity = 1}) {
    ...
    if (qty < minQuantity) return 'Số lượng tối thiểu là $minQuantity';
    return null;
  }
  ...
  static String? address(String? value) {
    ...
    if (value.trim().length < 10) return 'Địa chỉ phải đủ chi tiết (tối thiểu 10 ký tự)';
    return null;
  }
}
